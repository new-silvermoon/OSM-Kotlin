...
public abstract class OsmMapViewBase extends SurfaceView implements android.view.GestureDetector.OnGestureListener, OnDoubleTapListener  {
    ...
    public GeoPoint getProjectionFromPixels(int x, int y) {
        return Projection.Companion.getProjectionFromPixels(x, y, mZoomLevel);
    }

    public void setCenter(double lat, double lon, int mapWidth, int mapHeight) {

        int x = Projection.Companion.getXPixelFromLongitude(lon, mZoomLevel);
        int y = Projection.Companion.getYPixelFromLatitude(lat, mZoomLevel);
...
//		}
    ...
    private Tile[] initializeCurrentTiles(int zoomLevel, int offsetX, int offsetY) {
        ...
        for (int index = 0; mTiles != null && index < mTiles.length; ++index) {
            ...
            if (mTiles[index].getMapX() != (mapX + mIncrementsX[index])
                    || mTiles[index].getMapY() != (mapY + mIncrementsY[index])
                    || mTiles[index].getZoom() != zoomLevel) {
                mTiles[index].setMapX( mapX + mIncrementsX[index]);
                mTiles[index].setMapY( mapY + mIncrementsY[index]);
                mTiles[index].setOffsetX(mTiles[index].getMapX() * TILE_SIZE);
                mTiles[index].setOffsetY(mTiles[index].getMapY() * TILE_SIZE);
                mTiles[index].setZoom(zoomLevel);
                mTiles[index].setKey((zoomLevel + "/" + mTiles[index].getMapX() + "/"
                        + mTiles[index].getMapY() + ".png").intern());
                mTiles[index].setMapTypeId(mMaptTypeId);
            }
        }
        return mTiles;
    }
    ...
    private Tile[] initializePendingTiles(int zoomLevel, int offsetX, int offsetY) {
        ...
        for (int index = 0; mTiles != null && index < mTiles.length; ++index) {
            ...
            tiles[index].setMapX( mapX + mIncrementsX[index]);
            tiles[index].setMapY( mapY + mIncrementsY[index]);
            tiles[index].setOffsetX(mTiles[index].getMapX() * TILE_SIZE);
            tiles[index].setOffsetY(mTiles[index].getMapY() * TILE_SIZE);
            tiles[index].setZoom(zoomLevel);
            tiles[index].setKey((zoomLevel + "/" + tiles[index].getMapX() + "/"
                    + tiles[index].getMapY() + ".png").intern());
            tiles[index].setMapTypeId(mMaptTypeId);
        }
    ...
    private boolean isOnScreen(Tile tile) {
        ...
        int upperLeftX = tile.getOffsetX() + this.mOffsetX;
        int upperLeftY = tile.getOffsetY() + this.mOffsetY;
    ...
    private boolean isSane(Tile tile) {
        if (tile.getMapX() >= 0 && tile.getMapY() >= 0
                && tile.getMapX() <= (Math.pow(2, mZoomLevel) - 1)
                && tile.getMapY() <= (Math.pow(2, mZoomLevel) - 1)) {
            return true;
        }
        return false;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = (mTiles.length-1); i >= 0; i--) {
            if (isOnScreen(mTiles[i])) {
                ...
                if (bitmap != null) {
                    canvas.drawBitmap(bitmap, mOffsetX + mTiles[i].getOffsetX(), mOffsetY + mTiles[i].getOffsetY(), null);
                }
            }
        }
